...
public class ReservationController {
    ...
    public String showSeatSelection(@PathVariable Integer seanceId, Model model) {
        ...
        seatsByRow.values().forEach(seats -> 
            seats.sort(Comparator.comparing(Siege::getNumero))
        );
        ...
        try {
            ...
            for (Integer seatId : seatIds) {
                ...
                if (!existingBillets.isEmpty()) {
                    throw new RuntimeException("Le siège " + siege.getRangee() + siege.getNumero() + " est déjà réservé");
                }
                ...
                if (prixOriginal == null) {
                    throw new RuntimeException("Prix non défini pour le type de siège: " + siege.getTypeSiege().getNom());
                }
                
                // Chercher la meilleure remise applicable pour ce type de siège et l'âge du client
                Remise remiseApplicable = null;
                Integer prixFinal = prixOriginal;
                
                if (ageClient != null) {
                    Optional<Remise> meilleureRemise = remiseService.getMeilleureRemiseForTypeSiegeAndAge(typeSiegeId, ageClient);
